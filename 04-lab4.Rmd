...
```{r fig1, echo=FALSE, fig.cap='Sistema em malha-fechada com perturbação na saída.', out.width='100%'}
...
Agora, considere o sistema mostrado na Figura \@ref(fig:fig2) e assuma que $G_2(s)$ não possui *zeros* em $s=0$. Para tal sistema, a Tabela \@ref(tab:tab1) continua válida para $G(s) = G_1(s)G_2(s)$. No entanto, a Tabela \@ref(tab:tab2) deve ser substituída pela Tabela \@ref(tab:tab3). Ressaltamos que os valores $\neq 0$ na Tabela \@ref(tab:tab3) podem ser calculados analiticamente a partir de $G_1(s) \text{ e } G_2(s)$. Entretanto, isso não é o objeto de estudo desta experiência.

```{r fig2, echo=FALSE, fig.cap='Sistema em malha-fechada com perturbação na entrada de $G_s(s)$ e $G_2(s)$ não possui *zeros* em $s=0$.', out.width='100%'}
...
```{r fig3, echo=FALSE, fig.cap='Sistema em malha-fechada.', out.width='100%'}
